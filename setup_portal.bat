@echo off
echo Creating AUY Student Portal source files...
echo.
...
echo import { motion } from 'framer-motion';
echo import type { Variants } from 'framer-motion';
echo import { BookOpen, ExternalLink, User as UserIcon, CheckCircle2 } from 'lucide-react';
echo import { useData } from '../contexts/DataContext';
echo import { GlassCard, SectionTitle, Spinner, GradeColor } from '../components/Common';
echo.
echo const stagger: Variants = ^{
echo   hidden: {},
echo   show: { transition: { staggerChildren: 0.06 } },
echo ^};
echo.
echo const fadeUp: Variants = ^{
echo   hidden: { opacity: 0, y: 20 },
echo   show: { opacity: 1, y: 0, transition: { duration: 0.45, ease: [0.25, 0.46, 0.45, 0.94] } },
echo ^};
echo.
echo export default function Courses(^) ^{
echo   const { courses, loading } = useData(^);
echo.
echo   if (loading) return ^<Spinner size="lg" /^>;
echo.
echo   return (
echo     ^<motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6"^>
echo       ^<motion.div variants={fadeUp}^>
echo         ^<SectionTitle subtitle={`${courses.length} enrolled courses`}^>My Courses^</SectionTitle^>
echo       ^</motion.div^>
echo.
echo       ^<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"^>
echo         {courses.map((course^) =^> (
echo           ^<motion.div key={course.courseId} variants={fadeUp}^>
echo             ^<GlassCard className="p-6 h-full flex flex-col" hover^>
echo               ^<div className="flex items-start justify-between mb-4"^>
echo                 ^<div className="w-10 h-10 rounded-2xl bg-emerald-500/15 flex items-center justify-center shrink-0"^>
echo                   ^<BookOpen className="w-5 h-5 text-emerald-300" /^>
echo                 ^</div^>
echo                 ^<GradeColor grade={course.grade} /^>
echo               ^</div^>
echo.
echo               ^<h3 className="text-sm font-semibold text-white/90 mb-1 leading-snug"^>
echo                 {course.courseName}
echo               ^</h3^>
echo               ^<p className="text-xs text-white/40 mb-4"^>{course.courseId}^</p^>
echo.
echo               ^<div className="space-y-2 flex-1"^>
echo                 ^<div className="flex items-center gap-2 text-xs text-white/50"^>
echo                   ^<UserIcon className="w-3.5 h-3.5" /^>
echo                   ^<span^>{course.teacherName}^</span^>
echo                 ^</div^>
echo                 ^<div className="flex items-center gap-2 text-xs text-white/50"^>
echo                   ^<BookOpen className="w-3.5 h-3.5" /^>
echo                   ^<span^>{course.credits} Credits^</span^>
echo                 ^</div^>
echo                 ^<div className="flex items-center gap-2 text-xs text-white/50"^>
echo                   ^<CheckCircle2 className="w-3.5 h-3.5" /^>
echo                   ^<span^>Attendance: {course.attendancePercentage}%^</span^>
echo                 ^</div^>
echo               ^</div^>
echo.
echo               ^<a
echo                 href={course.googleClassroomLink}
echo                 target="_blank"
echo                 rel="noopener noreferrer"
echo                 className="mt-4 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/15 text-emerald-300 text-xs font-medium hover:bg-emerald-500/25 transition-colors"
echo               ^>
echo                 ^<ExternalLink className="w-3.5 h-3.5" /^>
echo                 Open Classroom
echo               ^</a^>
echo             ^</GlassCard^>
echo           ^</motion.div^>
...
echo import { motion } from 'framer-motion';
echo import type { Variants } from 'framer-motion';
echo import { Award, BookOpen, CheckCircle2 } from 'lucide-react';
echo import { useData } from '../contexts/DataContext';
echo import { GlassCard, SectionTitle, Spinner, GradeColor } from '../components/Common';
echo import { GRADE_POINTS } from '../services/firebaseData';
echo.
echo const stagger: Variants = ^{
echo   hidden: {},
echo   show: { transition: { staggerChildren: 0.06 } },
...
echo ^};
echo.
echo function gradeRowBg(grade: string^): string ^{
echo   if (grade.startsWith('A'^)) return 'bg-emerald-500/[0.04]';
echo   if (grade.startsWith('B'^)) return 'bg-blue-500/[0.04]';
echo   if (grade.startsWith('C'^)) return 'bg-yellow-500/[0.04]';
echo   if (grade.startsWith('D'^)) return 'bg-orange-500/[0.04]';
echo   if (grade === 'F') return 'bg-red-500/[0.04]';
echo   return '';
echo ^}
echo.
echo export default function Grades(^) ^{
echo   const { courses, gpa, totalCredits, loading } = useData(^);
echo.
echo   if (loading) return ^<Spinner size="lg" /^>;
echo.
echo   const completedCourses = courses.filter(c =^> c.grade ^&^& GRADE_POINTS[c.grade] !== undefined^);
echo.
echo   return (
echo     ^<motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6"^>
echo       ^<motion.div variants={fadeUp}^>
echo         ^<SectionTitle subtitle="Academic performance overview"^>Grades^</SectionTitle^>
echo       ^</motion.div^>
echo.
echo       ^<div className="grid grid-cols-1 sm:grid-cols-3 gap-4"^>
echo         ^<motion.div variants={fadeUp}^>
echo           ^<GlassCard className="p-5 flex items-center gap-4" hover^>
echo             ^<div className="w-11 h-11 rounded-2xl bg-emerald-500/15 flex items-center justify-center shrink-0"^>
echo               ^<Award className="w-5 h-5 text-emerald-300" /^>
echo             ^</div^>
echo             ^<div^>
echo               ^<p className="text-2xl font-bold text-white/90"^>{gpa.toFixed(2^)}^</p^>
echo               ^<p className="text-xs text-white/40"^>Cumulative GPA^</p^>
echo             ^</div^>
echo           ^</GlassCard^>
echo         ^</motion.div^>
echo.
echo         ^<motion.div variants={fadeUp}^>
echo           ^<GlassCard className="p-5 flex items-center gap-4" hover^>
echo             ^<div className="w-11 h-11 rounded-2xl bg-cyan-500/15 flex items-center justify-center shrink-0"^>
echo               ^<BookOpen className="w-5 h-5 text-cyan-300" /^>
echo             ^</div^>
echo             ^<div^>
echo               ^<p className="text-2xl font-bold text-white/90"^>{totalCredits}^</p^>
echo               ^<p className="text-xs text-white/40"^>Total Credits^</p^>
echo             ^</div^>
echo           ^</GlassCard^>
echo         ^</motion.div^>
echo.
echo         ^<motion.div variants={fadeUp}^>
echo           ^<GlassCard className="p-5 flex items-center gap-4" hover^>
echo             ^<div className="w-11 h-11 rounded-2xl bg-teal-500/15 flex items-center justify-center shrink-0"^>
echo               ^<CheckCircle2 className="w-5 h-5 text-teal-300" /^>
echo             ^</div^>
echo             ^<div^>
echo               ^<p className="text-2xl font-bold text-white/90"^>{completedCourses.length}^</p^>
echo               ^<p className="text-xs text-white/40"^>Courses Completed^</p^>
echo             ^</div^>
echo           ^</GlassCard^>
echo         ^</motion.div^>
echo       ^</div^>
echo.
echo       ^<motion.div variants={fadeUp}^>
echo         ^<GlassCard className="overflow-hidden"^>
echo           ^<div className="hidden sm:block overflow-x-auto"^>
echo             ^<table className="w-full"^>
echo               ^<thead^>
echo                 ^<tr className="border-b border-white/[0.06]"^>
echo                   ^<th className="text-left text-xs font-medium text-white/40 uppercase tracking-wider px-6 py-4"^>Course^</th^>
echo                   ^<th className="text-left text-xs font-medium text-white/40 uppercase tracking-wider px-6 py-4"^>Code^</th^>
echo                   ^<th className="text-center text-xs font-medium text-white/40 uppercase tracking-wider px-6 py-4"^>Credits^</th^>
echo                   ^<th className="text-center text-xs font-medium text-white/40 uppercase tracking-wider px-6 py-4"^>Grade^</th^>
echo                   ^<th className="text-center text-xs font-medium text-white/40 uppercase tracking-wider px-6 py-4"^>Points^</th^>
echo                 ^</tr^>
echo               ^</thead^>
echo               ^<tbody^>
echo                 {courses.map((course^) =^> (
echo                   ^<tr
echo                     key={course.courseId}
echo                     className={`border-b border-white/[0.04] last:border-0 ${gradeRowBg(course.grade^)} transition-colors hover:bg-white/[0.03]`}
echo                   ^>
echo                     ^<td className="px-6 py-4 text-sm text-white/80 font-medium"^>{course.courseName}^</td^>
echo                     ^<td className="px-6 py-4 text-sm text-white/50"^>{course.courseId}^</td^>
echo                     ^<td className="px-6 py-4 text-sm text-white/60 text-center"^>{course.credits}^</td^>
echo                     ^<td className="px-6 py-4 text-center"^>
echo                       ^<GradeColor grade={course.grade} /^>
echo                     ^</td^>
echo                     ^<td className="px-6 py-4 text-sm text-white/60 text-center"^>
echo                       {course.grade ^&^& GRADE_POINTS[course.grade] !== undefined
echo                         ? (GRADE_POINTS[course.grade] * course.credits^).toFixed(1^)
echo                         : '\u2014'}
echo                     ^</td^>
echo                   ^</tr^>
echo                 ^)^)}
...
echo             ^</table^>
echo           ^</div^>
echo           ^<div className="sm:hidden divide-y divide-white/[0.06]"^>
echo             {courses.map((course^) =^> (
echo               ^<div key={course.courseId} className={`p-4 ${gradeRowBg(course.grade^)}`}^>
echo                 ^<div className="flex items-start justify-between mb-2"^>
echo                   ^<div className="min-w-0 flex-1"^>
echo                     ^<p className="text-sm font-medium text-white/85 truncate"^>{course.courseName}^</p^>
echo                     ^<p className="text-xs text-white/40"^>{course.courseId}^</p^>
echo                   ^</div^>
echo                   ^<GradeColor grade={course.grade} /^>
echo                 ^</div^>
echo                 ^<div className="flex gap-4 text-xs text-white/40"^>
echo                   ^<span^>{course.credits} credits^</span^>
echo                   ^<span^>
echo                     {course.grade ^&^& GRADE_POINTS[course.grade] !== undefined
echo                       ? `${(GRADE_POINTS[course.grade] * course.credits^).toFixed(1^)} pts`
echo                       : '\u2014'}
echo                   ^</span^>
echo                 ^</div^>
echo               ^</div^>
...
echo import { motion } from 'framer-motion';
echo import type { Variants } from 'framer-motion';
echo import { FolderOpen, Upload, FileText, BookOpen } from 'lucide-react';
echo import { GlassCard, SectionTitle } from '../components/Common';
echo.
echo const fadeUp: Variants = ^{
echo   hidden: { opacity: 0, y: 20 },
echo   show: { opacity: 1, y: 0, transition: { duration: 0.5, ease: [0.25, 0.46, 0.45, 0.94] } },
echo ^};
echo.
echo export default function Materials(^) ^{
echo   return (
echo     ^<motion.div
echo       initial="hidden"
echo       animate="show"
echo       variants={{ show: { transition: { staggerChildren: 0.1 } } }}
echo       className="space-y-6"
echo     ^>
echo       ^<motion.div variants={fadeUp}^>
echo         ^<SectionTitle subtitle="Access your course materials"^>Study Materials^</SectionTitle^>
echo       ^</motion.div^>
echo.
echo       ^<motion.div variants={fadeUp}^>
echo         ^<GlassCard className="p-12 flex flex-col items-center justify-center text-center"^>
echo           ^<div className="w-20 h-20 rounded-3xl bg-emerald-500/10 flex items-center justify-center mb-6"^>
echo             ^<FolderOpen className="w-10 h-10 text-emerald-400/60" /^>
echo           ^</div^>
echo           ^<h3 className="text-lg font-semibold text-white/80 mb-2"^>No Materials Yet^</h3^>
echo           ^<p className="text-sm text-white/40 max-w-md leading-relaxed"^>
echo             Study materials will appear here once uploaded by your instructors.
echo             Check back soon or visit your Google Classroom links for the latest resources.
echo           ^</p^>
echo         ^</GlassCard^>
echo       ^</motion.div^>
echo.
...
echo         ^<div className="grid grid-cols-1 sm:grid-cols-3 gap-4"^>
echo           {[
echo             { icon: FileText, label: 'Lecture Notes', desc: 'Coming soon' },
echo             { icon: BookOpen, label: 'Textbooks', desc: 'Coming soon' },
echo             { icon: Upload, label: 'Assignments', desc: 'Coming soon' },
echo           ].map((item^) =^> (
echo             ^<GlassCard key={item.label} className="p-5 flex items-center gap-4 opacity-50" hover={false}^>
echo               ^<div className="w-10 h-10 rounded-xl bg-white/[0.06] flex items-center justify-center shrink-0"^>
echo                 ^<item.icon className="w-5 h-5 text-white/40" /^>
echo               ^</div^>
echo               ^<div^>
echo                 ^<p className="text-sm font-medium text-white/60"^>{item.label}^</p^>
echo                 ^<p className="text-xs text-white/30"^>{item.desc}^</p^>
echo               ^</div^>
echo             ^</GlassCard^>
echo           ^)^)}
echo         ^</div^>
...
echo import { motion } from 'framer-motion';
echo import type { Variants } from 'framer-motion';
echo import { Target, TrendingUp, Calendar, Award, CheckCircle2, BookOpen } from 'lucide-react';
echo import { useData } from '../contexts/DataContext';
echo import { GlassCard, SectionTitle, Spinner } from '../components/Common';
echo import { GRADE_POINTS } from '../services/firebaseData';
echo.
echo const stagger: Variants = ^{
echo   hidden: {},
echo   show: { transition: { staggerChildren: 0.1 } },
echo ^};
echo.
echo const fadeUp: Variants = ^{
echo   hidden: { opacity: 0, y: 20 },
...
echo ^};
echo.
echo interface ProgressBarProps ^{
echo   label: string;
echo   value: number;
echo   max: number;
echo   unit?: string;
...
echo ^}
echo.
echo function ProgressBar({ label, value, max, unit, icon, color, delay = 0 }: ProgressBarProps) ^{
echo   const pct = Math.min((value / max^) * 100, 100^);
echo   return (
echo     ^<GlassCard className="p-6" hover^>
echo       ^<div className="flex items-center gap-3 mb-4"^>
echo         ^<div className="w-10 h-10 rounded-2xl bg-white/[0.06] flex items-center justify-center shrink-0"^>
echo           {icon}
echo         ^</div^>
echo         ^<div className="flex-1"^>
echo           ^<p className="text-sm font-medium text-white/80"^>{label}^</p^>
echo           ^<p className="text-xs text-white/40"^>
echo             {value.toFixed(label.includes('GPA'^) ? 2 : 0^)}{unit ? ` ${unit}` : ''} / {max}{unit ? ` ${unit}` : ''}
echo           ^</p^>
echo         ^</div^>
echo         ^<span className="text-2xl font-bold text-white/90"^>{pct.toFixed(0^)}%^</span^>
echo       ^</div^>
echo       ^<div className="h-3 bg-white/[0.06] rounded-full overflow-hidden"^>
echo         ^<motion.div
echo           className={`h-full rounded-full ${color}`}
echo           initial={{ width: 0 }}
echo           animate={{ width: `${pct}%` }}
echo           transition={{ duration: 1.4, ease: [0.25, 0.46, 0.45, 0.94], delay }}
echo         /^>
echo       ^</div^>
echo     ^</GlassCard^>
echo   );
echo ^}
echo.
echo export default function Progress(^) ^{
echo   const { courses, gpa, totalCredits, averageAttendance, loading } = useData(^);
echo.
echo   if (loading) return ^<Spinner size="lg" /^>;
echo.
echo   const completedCourses = courses.filter(c =^> c.grade ^&^& GRADE_POINTS[c.grade] !== undefined^).length;
echo   const totalCourses = courses.length;
echo.
echo   return (
echo     ^<motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6"^>
echo       ^<motion.div variants={fadeUp}^>
echo         ^<SectionTitle subtitle="Track your academic journey"^>Academic Progress^</SectionTitle^>
echo       ^</motion.div^>
echo.
echo       ^<div className="grid grid-cols-1 lg:grid-cols-2 gap-4"^>
echo         ^<motion.div variants={fadeUp}^>
echo           ^<ProgressBar
echo             label="Credit Completion"
echo             value={totalCredits}
echo             max={124}
echo             unit="credits"
echo             icon={<Target className="w-5 h-5 text-emerald-300" /^>}
echo             color="bg-gradient-to-r from-emerald-500 to-emerald-300"
echo             delay={0.1}
echo           /^>
echo         ^</motion.div^>
echo.
echo         ^<motion.div variants={fadeUp}^>
echo           ^<ProgressBar
echo             label="GPA Progress"
echo             value={gpa}
echo             max={4}
echo             icon={<TrendingUp className="w-5 h-5 text-cyan-300" /^>}
echo             color="bg-gradient-to-r from-cyan-500 to-cyan-300"
echo             delay={0.2}
echo           /^>
echo         ^</motion.div^>
echo.
echo         ^<motion.div variants={fadeUp}^>
echo           ^<ProgressBar
echo             label="Average Attendance"
echo             value={averageAttendance}
echo             max={100}
echo             unit="%"
echo             icon={<Calendar className="w-5 h-5 text-teal-300" /^>}
echo             color="bg-gradient-to-r from-teal-500 to-teal-300"
echo             delay={0.3}
echo           /^>
echo         ^</motion.div^>
echo.
echo         ^<motion.div variants={fadeUp}^>
echo           ^<ProgressBar
echo             label="Courses Completed"
echo             value={completedCourses}
echo             max={Math.max(totalCourses, 1^)}
echo             icon={<BookOpen className="w-5 h-5 text-violet-300" /^>}
echo             color="bg-gradient-to-r from-violet-500 to-violet-300"
echo             delay={0.4}
echo           /^>
echo         ^</motion.div^>
echo       ^</div^>
echo.
echo       ^<motion.div variants={fadeUp}^>
echo         ^<SectionTitle subtitle="Key achievements" className="mt-2"^>Milestones^</SectionTitle^>
echo         ^<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4"^>
echo           {[
echo             ^{
echo               icon: <Award className="w-6 h-6 text-emerald-300" /^>,
echo               label: "Dean's List",
echo               status: gpa >= 3.5 ? 'Achieved' : 'In Progress',
echo               achieved: gpa >= 3.5,
echo               desc: 'GPA \u2265 3.50',
echo             },
echo             ^{
echo               icon: <CheckCircle2 className="w-6 h-6 text-cyan-300" /^>,
echo               label: 'Perfect Attendance',
echo               status: averageAttendance >= 95 ? 'Achieved' : 'In Progress',
echo               achieved: averageAttendance >= 95,
echo               desc: 'Attendance \u2265 95%',
echo             },
echo             ^{
echo               icon: <Target className="w-6 h-6 text-teal-300" /^>,
echo               label: 'Halfway There',
echo               status: totalCredits >= 62 ? 'Achieved' : 'In Progress',
echo               achieved: totalCredits >= 62,
echo               desc: '62+ credits earned',
echo             },
echo             ^{
echo               icon: <TrendingUp className="w-6 h-6 text-violet-300" /^>,
echo               label: 'Honor Roll',
echo               status: gpa >= 3.0 ? 'Achieved' : 'In Progress',
echo               achieved: gpa >= 3.0,
echo               desc: 'GPA \u2265 3.00',
echo             },
echo           ].map((m^) =^> (
echo             ^<GlassCard
echo               key={m.label}
echo               className={`p-5 text-center ${m.achieved ? 'ring-1 ring-emerald-500/20' : 'opacity-60'}`}
echo               hover
echo             ^>
echo               ^<div className="w-12 h-12 rounded-2xl bg-white/[0.06] flex items-center justify-center mx-auto mb-3"^>
echo                 {m.icon}
echo               ^</div^>
echo               ^<p className="text-sm font-semibold text-white/85"^>{m.label}^</p^>
echo               ^<p className="text-[11px] text-white/40 mt-0.5"^>{m.desc}^</p^>
echo               ^<span
echo                 className={`inline-block mt-2 text-[10px] font-medium px-2.5 py-1 rounded-full ${
echo                   m.achieved
echo                     ? 'bg-emerald-500/20 text-emerald-300'
echo                     : 'bg-white/[0.06] text-white/40'
echo                 }`}
echo               ^>
echo                 {m.status}
echo               ^</span^>
...
:: ==================== services/firebaseData.ts ====================
mkdir src\services 2>nul
(
echo export const GRADE_POINTS: Record^<string, number^> = ^{
echo   'A': 4.0, 'A-': 3.7,
echo   'B+': 3.3, 'B': 3.0, 'B-': 2.7,
echo   'C+': 2.3, 'C': 2.0, 'C-': 1.7,
echo   'D+': 1.3, 'D': 1.0,
